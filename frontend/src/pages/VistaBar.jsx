...
import { actualizarEstadoPedido } from '../utils/ApiPedidos'; // función común
...
const VistaBar = () => {
  ...
  const cargarPedidos = () => {
    axios.get(`${process.env.REACT_APP_API}/api/pedidos/activos`)
      .then(res => {
        const categoriasBebestibles = ['bebida', 'jugo', 'agua', 'cerveza', 'preparado', 'destilado'];

        const pedidosBar = res.data
          .filter(p => p.estado !== 'esperando cuenta') // oculta los que piden la cuenta
          .map(pedido => ({
            ...pedido,
            items: pedido.items.filter(item =>
              categoriasBebestibles.includes(item.productoId?.categoria?.toLowerCase())
            )
          }))
          .filter(pedido => pedido.items.length > 0);

        setPedidosActivos(pedidosBar);
      })
      .catch(err => console.error("Error al cargar pedidos activos para el bar:", err));
  };
  ...
  return (
    <div className="min-h-screen bg-black text-white p-6"
      ...
      <h1 className="text-3xl font-bold mb-6 text-center">Vista Bar</h1>
